...
class dataset():
    ...
    def plotter(self):
        plt.xlabel('t (s)') # common label on the x-axis #plt.show() 
        self.fig.savefig(self.filename+'_plot.png')  #figname              
    ...
    def model(self):   
        ...
        #by-eye estimates of interval in which said value is to be found
        #t0 = np.linspace(3600, 3850, n) #Time when velocity is maximal 
        #vr = np.linspace(32, 58, n)     #radial velocity is highest and/or lowest
        #P = np.linspace(3400,5000, n)   #Orbital period 
        ...
        for i in t0: 
            for j in vr: 
                for k in P:                     
                    ...
                    # Calculate model for radial velocity 
                    model = j*cos((2*pi/k*(i-t)))  #fmax + (i - fmax)*(exp(-(l - k)**2.0/(2.0*(j**2.0))))  #t0[0]
                    #print "tempmodel=",model
                    # Calculate change                    
                    deltaCurrent = sum((self.v_rel - model)**2) #(observed - model)**2 
                    #print "deltacurrent=",deltaCurrent
                    ...
                    if deltaCurrent < best_delta: 
                        ...
                        delta[1] = j 
                        delta[2] = k 
        self.modelP, self.modelV_rel = delta[2], delta[1]
    ...
    def mass(self, v, P):
        m_sun = 1.98892*10**30
        G = 6.67428*10**(-11)       
        self.planetMass = ((self.starMass * m_sun)**(0.6666667)*v*P**(0.3333333))/((2*pi*G)**(0.3333333))
